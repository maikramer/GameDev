...
- Commands on `PATH` per workflow (install packages in their environments) or environment variables:
  ...
  - `TEXTURE2D_BIN` — `texture2d` executable ([Texture2D](../Texture2D)) when using **seamless textures** (`image_source: texture2d` or YAML field)
  - `TEXT3D_BIN` — `text3d` executable ([Text3D](../Text3D)) when 3D is enabled via `generate_3d` column
  ...
  - `TEXT2SOUND_BIN` — `text2sound` executable ([Text2Sound](../Text2Sound)) when manifest rows have **`generate_audio=true`** (and you do not use `--skip-audio`)
  ...
  - `PART3D_BIN` — `part3d` executable ([Part3D](../Part3D)) when Part3D is enabled via `part3d` profile block or YAML field **`generate_parts=true`** (semantic decomposition after Text3D GLB)
  - `ANIMATOR3D_BIN` — `animator3d` executable ([Animator3D](../Animator3D)) after a successful rig (`animator3d game-pack`; auto-detected when `rigging3d` profile block exists — see batch section below)
...
|------------|-------------|
| `gameassets init` | Creates `game.yaml` and `manifest.yaml` in a folder |
...
Creates `game.yaml` (profile) and `manifest.yaml` (asset list).
...
**2D + 3D** where `generate_3d=true` in the manifest:
...
- Pipeline stages (3D, rig, parts, animate) are **auto-detected** from the manifest YAML fields (`generate_3d`, `generate_rig`, `generate_parts`, `generate_animate`) and `game.yaml` profile blocks (`rigging3d`, `part3d`, `animator3d`). Use **`--no-3d`**, **`--no-rig`**, **`--no-parts`**, **`--no-animate`** to explicitly opt out of a stage. For example, a `rigging3d: {}` block in `game.yaml` enables rigging for character rows even without `generate_rig=true` in the manifest.
- With 3D enabled and a **`rigging3d`** profile block (or `generate_rig=true`), rows with rigging eligibility call **Rigging3D**; rigged GLB appears in the log as **`rig_mesh_path`** (suffix configurable via `rigging3d.output_suffix` in `game.yaml`). Characters (`kind: character`) are auto-rigged when the profile has a `rigging3d` block. Requires **`RIGGING3D_BIN`** or `rigging3d` on `PATH`.
- After a successful rig, the batch runs **`animator3d game-pack`** for animation-eligible rows: explicit **`generate_animate=true`**, or rigged rows when the profile has **`animator3d`** configured (Animator3D profile from optional **`animator3d`** in `game.yaml`, default preset `humanoid`). Requires **`ANIMATOR3D_BIN`** or `animator3d` on `PATH`. See [Animator3D after rig](../docs/ANIMATOR3D_AFTER_RIG.md).
- With 3D enabled and a **`part3d`** profile block (or `generate_parts=true`), eligible rows call **Part3D** (`part3d decompose`) on the Text3D GLB **before** rig: outputs **`parts_mesh_path`** (multi-part scene) and **`segmented_mesh_path`** (per-part colors), alongside the main GLB; options under **`part3d`** in `game.yaml`. Requires **`PART3D_BIN`** or `part3d` on `PATH`.
...
- Optional YAML field **`generate_audio`** (`true`/`false`): when `true`, after 2D image (phase 1) runs **Text2Sound** (phase 1b) before Text3D.
...
**Per manifest entry:** optional **`image_source`** (`text2d` or `texture2d`) overrides the profile default for that row (mix FLUX props and Texture2D tiles in one manifest).
...
Minimal example (with `generate_3d=true` rows):
...
### Asset categories and mesh targets

Each asset is classified into a category (via `infer_category()` keyword matching or explicit `category` field). The category determines the **target face count** used by `text3d remesh-textured` (isotropic remesh + texture reprojection) when decimating high-poly painted meshes to game-ready resolution.

| Category | Default kind | Target faces | Notes |
|----------|-------------|-------------|-------|
| humanoid | character | 16,000 | Humanoids, goblins, orcs — enough faces for clean rigging/skinning |
| building | environment | 12,000 | Large structures with architectural detail |
| creature | character | 12,000 | Animals, monsters, dragons |
| vehicle | prop | 8,000 | Mechanical geometry (wheels, body) |
| terrain | environment | 8,000 | Ground tiles with surface layers |
| tree | environment | 6,000 | Trunk + canopy separation |
| armor | prop | 5,000 | Plate detail and straps |
| vegetation | environment | 5,000 | Leaves, stems, flowers |
| chest | prop | 4,000 | Lid hinge and lock detail |
| furniture | prop | 4,000 | Legs, backrest, armrests |
| weapon | prop | 3,000 | Blade edge and grip |
| mineral | prop | 2,400 | Crystal facets and shards |
| tool | prop | 2,000 | Handle and working head |
| rock | prop | 1,600 | Simple boulder geometry |
| food | prop | 1,600 | Organic shapes |
| effects | prop | 1,000 | Abstract low-poly elements |

Categories and defaults are defined in `src/gameassets/categories.py`. The `infer_category()` function maps keywords (including Portuguese terms like `goblin`, `orc`, `guerreiro`, `dragao`) to the appropriate category.
...
| `image_source` | `text2d` (default), `texture2d`, or `skymap2d` — default image tool (overridable per YAML field) |
| `text2d` | Optional block: `low_vram`, `cpu`, `width`, `height` |
| `texture2d` | Optional block when using Texture2D (global or manifest `texture2d` lines): resolution, `steps`, `guidance_scale`, `preset`, … and **PBR from diffuse:** `materialize`, `materialize_maps_subdir`, `materialize_bin`, `materialize_format`, etc. |
| `text3d` | Optional block: `preset`, `low_vram`, `texture` (omitted = **`true`**), `steps` / `octree_resolution` / `num_chunks` (mutually exclusive with `preset` in practice), `mc_level`, `phased_batch`, `allow_shared_gpu`, `gpu_kill_others`, `full_gpu`, `model_subfolder`. **Paint3D tuning:** `paint_max_views`, `paint_view_resolution`, `paint_render_size`, `paint_texture_size`, `paint_bake_exp` (default 6 — sharper view blending) |
| `rigging3d` | Optional block (rig after Text3D): `output_suffix` (e.g. `_rigged`), `root` (Rigging3D package code), `python` (interpreter). Presence enables rigging for character rows; combine with `generate_rig=true` in the manifest for non-character rows |
| `animator3d` | Optional block (**Animator3D** after rig): `preset` (`humanoid` \| `creature` \| `flying`, …). Presence enables auto-animation after successful rig. Requires `animator3d` on `PATH` or `ANIMATOR3D_BIN` |
| `part3d` | Optional block (Part3D after Text3D, before rig): `steps`, `octree_resolution`, `num_chunks`, `segment_only`, `no_cpu_offload`, `verbose`, `parts_suffix`, `segmented_suffix`. Presence enables parts for 3D rows; combine with `generate_parts=true` in the manifest for explicit per-row control |
...
Headers: **`id`**, **`idea`** (required); optional: **`kind`** (`prop`, `character`, `environment`), **`generate_3d`**, **`generate_audio`**, **`generate_rig`** (`true`/`false`/… — rig GLB after Text3D; auto-enabled for characters when `rigging3d` profile block exists), **`generate_animate`** (`true`/`false`/… — run **Animator3D** after rig; auto-enabled for rigged rows when `animator3d` profile block exists), **`generate_parts`** (`true`/`false`/… — Part3D decomposition after Text3D; auto-enabled when `part3d` profile block exists), **`image_source`** (`text2d` \| `texture2d` \| `skymap2d`) to override `game.yaml` `image_source` for that row. With `path_layout: flat`, use `id` with slashes, e.g. `Crystals/shard_blue`, to write files under `Crystals/`.
...
GameAssets/
├── src/gameassets/
│   ├── cli.py             # Click CLI (init, prompts, batch, handoff, dream, info, …)
│   ├── profile.py         # game.yaml parsing
│   ├── manifest.py        # manifest.yaml parsing
...
| `RIGGING3D_BIN` | Path to `rigging3d` (or `python -m rigging3d`) when rigging is enabled |
| `ANIMATOR3D_BIN` | Path to `animator3d` when animation is enabled |
| `PART3D_BIN` | Path to `part3d` (or `python -m part3d`) when parts is enabled |
