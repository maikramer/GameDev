...
function parseVector3Component(value: unknown, axis: 0 | 1 | 2): number {
  ...
  if (Array.isArray(value) && value.length > axis && typeof value[axis] === 'number') {
    return value[axis];
  }
...
export const tweenParser: Parser = ({ entity, element, state, context }) => {
  ...
  if (typeof attr === 'string') {
    ...
    else if (attr === 'rigidbody.pos-y' || attr === 'pos-y') axis = TweenAxis.PosY;
    else if (attr === 'rigidbody.pos-z' || attr === 'pos-z') axis = TweenAxis.PosZ;
  ...
  const vectorAxis = isRotationAxis
    ? (axis === TweenAxis.RotX ? 0 : axis === TweenAxis.RotY ? 1 : 2) as 0 | 1 | 2
    : 0;

  state.addComponent(entity, TweenData);
  TweenData.targetEntity[entity] = targetEid;
  TweenData.axis[entity] = axis;
  TweenData.from[entity] = isRotationAxis ? parseVector3Component(from, vectorAxis) : toNum(from, 0);
  TweenData.to[entity] = isRotationAxis ? parseVector3Component(to, vectorAxis) : toNum(to, 0);
  ...
  if (typeof easing === 'string') {
    if (easing === 'ease-in-out') TweenData.easing[entity] = EasingType.EaseInOut;
    else if (easing === 'ease-out-quad') TweenData.easing[entity] = EasingType.EaseOutQuad;
