  ...
  createMaterial(context: TerrainMaterialContext): THREE.Material {
    ...
    material.onBeforeCompile = (shader) => {
      ...
      shader.vertexShader = shader.vertexShader.replace(
        ...
        float _nHL = texture2D(tHeightMap, _nGUV - vec2(_nTS, 0.0)).r;
        float _nHR = texture2D(tHeightMap, _nGUV + vec2(_nTS, 0.0)).r;
        float _nHD = texture2D(tHeightMap, _nGUV - vec2(0.0, _nTS)).r;
        float _nHU = texture2D(tHeightMap, _nGUV + vec2(0.0, _nTS)).r;
        float _sX = (_nHR - _nHL) * 0.5;
        float _sZ = (_nHU - _nHD) * 0.5;
