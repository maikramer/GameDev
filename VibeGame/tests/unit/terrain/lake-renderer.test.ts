...
import { createLakeWaterEntities, createRiverWaterEntities } from "../../../src/plugins/terrain/lake-renderer";
  ...
  describe("createLakeWaterEntities", () => {
    ...
    it("generates a single Water entity for one lake plane", () => {
      const data: TerrainData = {
        ...BASE_DATA,
        lake_planes: [
          { lake_id: 0, pos_x: 50.0, pos_y: 25.0, pos_z: 75.0, size_x: 12.5, size_z: 7.5 },
        ],
      };
    ...
    it("produces valid XML attribute format", () => {
      const data: TerrainData = {
        ...BASE_DATA,
        lake_planes: [
          { lake_id: 0, pos_x: 1.5, pos_y: 2.5, pos_z: 3.5, size_x: 4.5, size_z: 5.5 },
        ],
      };
  ...
  describe("createRiverWaterEntities", () => {
    ...
    it("generates a Water entity at river source for valid river", () => {
      const data: TerrainData = {
        ...BASE_DATA,
        rivers: [
          { id: 0, source: [512, 0], path: [[512, 0], [511, 1]], length: 10 },
        ],
      };

      const result = createRiverWaterEntities(data);

      expect(result).toBe(`<Water pos="512 0 0" size-x="2" size-z="2"></Water>`);
    });
    ...
    it("generates Water entities for multiple rivers", () => {
      const data: TerrainData = {
        ...BASE_DATA,
        rivers: [
          { id: 0, source: [100, 200], path: [[100, 200], [101, 201]], length: 5 },
          { id: 1, source: [300, 400], path: [[300, 400], [301, 401]], length: 8 },
        ],
      };

      const result = createRiverWaterEntities(data);

      expect(result).toContain(`<Water pos="100 0 200" size-x="2" size-z="2"></Water>`);
      expect(result).toContain(`<Water pos="300 0 400" size-x="2" size-z="2"></Water>`);
      expect(result.split("\n")).toHaveLength(2);
    });
    ...
    it("skips rivers with short paths but includes valid ones", () => {
      const data: TerrainData = {
        ...
        rivers: [
          ...
          { id: 1, source: [50, 60], path: [[50, 60], [51, 61]], length: 3 },
        ],
      };
      ...
      expect(result).toContain(`<Water pos="50 0 60" size-x="2" size-z="2"></Water>`);
    });
  });
});
