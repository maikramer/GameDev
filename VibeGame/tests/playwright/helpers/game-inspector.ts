...
export class GameInspector {
  ...
  async component(
    ...
    return this.page.evaluate(
      ([e, n]) => getBridge().component(e, n),
      [eid, name] as [number, string]
    );
  }

  async query(...componentNames: string[]): Promise<number[]> {
    return this.page.evaluate((names) => getBridge().query(...names), componentNames);
  }
...
export async function injectWebGLErrorCapture(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    proto.getContext = function (contextId: string, ...args: unknown[]) {
      const ctx = origGetContext(contextId, ...args) as WebGL2RenderingContext | null;
      if (contextId === 'webgl2' && ctx && 'getError' in ctx) {
        ...
        gl.compileShader = function (shader: WebGLShader) {
          ...
          if (!gl.getShaderParameter(shader, gl.COMPILE_STATUS)) {
            ...
            if (info) {
              (w.__VIBEGAME_WEBGL_ERRORS as string[]).push(`Shader compile: ${info}`);
            }
          }
        };
        ...
        gl.linkProgram = function (program: WebGLProgram) {
          ...
          if (!gl.getProgramParameter(program, gl.LINK_STATUS)) {
            ...
            if (info) {
              (w.__VIBEGAME_WEBGL_ERRORS as string[]).push(`Program link: ${info}`);
            }
          }
        };
      }
