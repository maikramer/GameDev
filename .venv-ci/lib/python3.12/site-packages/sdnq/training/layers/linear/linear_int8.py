...
def quantize_int_mm_input(input: torch.FloatTensor, scale: torch.FloatTensor = None, dim: int = -1, do_input_reshape: bool = True, use_sr: bool = False) -> tuple[torch.CharTensor, torch.FloatTensor]:
    if do_input_reshape:
